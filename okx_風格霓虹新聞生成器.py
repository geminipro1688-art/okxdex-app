...
def get_coingecko_image(query):
    ...
    try:
        ...
        st.warning(f"CoinGecko API 連線錯誤: {e}")
    return None

def download_image_as_base64(url, do_remove_bg=False):
    """下載圖片並轉為 Base64，可選是否去背"""
    try:
        response = requests.get(url, timeout=5)
...
with st.sidebar:
    ...
    with st.expander("📅 全域設定", expanded=True):
        col1, col2 = st.columns([2, 1])
        main_title = col1.text_input("主標題", "DEX 速報")
        date_str = col2.text_input("日期", datetime.now().strftime("%m/%d"))
        sub_title = st.text_input("副標題", "社群小編獨家整理")

    news_data = []

    # 生成 4 則新聞的輸入框
    for i in range(4):
        with st.expander(f"📰 新聞 #{i+1}", expanded=False):
            title = st.text_input(f"標題 #{i+1}", key=f"title_{i}")
            content = st.text_area(f"內文 #{i+1}", key=f"content_{i}", height=100)

            # 代幣圖片設定
            col_t1, col_t2 = st.columns([3, 1])
            token_query = col_t1.text_input(f"代幣名稱 (例如 BTC) #{i+1}", key=f"token_{i}")
            auto_fetch = col_t2.button("🔍 抓圖", key=f"btn_{i}")

            # 狀態管理 (Session State for image url)
            ...
            # 顯示/編輯圖片連結
            image_url = st.text_input(f"圖片網址 #{i+1}", value=st.session_state[img_key], key=f"input_url_{i}")

            # 去背選項
            remove_bg_option = st.checkbox("✂️ 自動去背 (AI)", value=True, key=f"bg_{i}")

            # 狀態圖示選擇
            status_icon = st.selectbox(
                f"狀態圖示 #{i+1}",
                ["無", "上漲 (Bull)", "下跌 (Bear)", "警告 (Alert)", "鎖倉 (Lock)", "解鎖 (Unlock)"],
                key=f"status_{i}"
            )
    ...
    with st.expander("📢 底部跑馬燈", expanded=True):
        selected_preset = st.selectbox("選擇常用金句", ["自訂..."] + FOOTER_PRESETS)
        if selected_preset == "自訂...":
            footer_text = st.text_input("自訂文字", "One More Thing: ...")
        else:
            footer_text = st.text_input("底部文字", selected_preset)

# === 主畫面：生成與預覽 ===

st.title("⚡ 霓虹新聞速報生成器")
st.markdown("填寫左側欄位，右側將即時生成預覽圖。")

if st.button("🚀 生成預覽圖 (Generate)", type="primary"):
    with st.spinner("正在處理圖片與去背中..."):
        # 預先處理圖片 Base64
        processed_news = []
        ...
        st.info("提示：您可以直接對上方預覽圖進行「右鍵 -> 另存圖片」或使用截圖工具保存。")